...
<?php require_once __DIR__ . '/../includes/footer.php'; ?>
 ...
 <body>
    ...
    <main class="container" style="margin-top: 40px; text-align: right;">
        <h2><?= $editing ? 'تعديل' : 'إضافة' ?> مستخدم</h2>
        <form action="user_action.php" method="POST" style="max-width: 500px; margin-top: 20px;">
            <input type="hidden" name="action" value="<?php $editing ? 'edit':'add'?>">
            <?php if ($editing): ?>
                <input type="hidden" name="id" value="<?php htmlspecialchars($user['User_ID']) ?>">
            <?php endif; ?>

            <div class="form-group">
                <label for="full_name" class="form-lable">الاسم الكامل</label>
                <input type="text" id="full_name" name="full_name" class="form-control" required value="<?php htmlspecialchars($user['FullName'] ?? '') ?>">
            </div>

             <div class="form-group">
                <label for="email" class="form-lable">البريد الإلكتروني</label>
                <input type="email" id="email" name="email" class="form-control" required value="<?php htmlspecialchars($user['Email'] ?? '') ?>">
            </div>
             <div class="form-group">
                <label for="phone" class="form-lable">رقم الهاتف</label>
                <input type="text" id="phone" name="phone" class="form-control" placeholder="0599000000 : مثال" value="<?php htmlspecialchars($user['PhoneNumber'] ?? '') ?>">
            </div>

             <div class="form-group">
                <label for="user_role" class="form-lable">دور و صلاحية المستخدم</label>
                <select name="user_role" id="user_role" class="form-controle" required>
                    <option value="EndUser" <?php ($user['UserRole'] ?? '') === 'EndUser' ? 'selected' : '' ?>>مستخدم نهائي (EndUser)</option>
                    <option value="Admin" <?php ($user['UserRole'] ?? '') ==='Admin' ? 'selected':'' ?>>مسؤول (Admin)</option>
                </select>    
            </div>

             <div class="form-group">
                <label for="password" class="form-lable">كلمة المرور <?php $editing ?? '(اتركه فارغأ للاحتفاظ بكلمة المرور دون تغيير)' ?></label>
                <input type="text" id="full_name" name="full_name" class="form-control" required value="<?php htmlspecialchars($user['FullName'] ?? '') ?>">

            </div>

            <div style="display: flex; gap: 10px; margin-top: 25px;">
                <button type="submit" class="btn btn-primary"><?php $editing ? 'تحديث' : 'إنشاء' ?> مستخدم</button>
                <a href="manage_users.php" class="btn btn-secondary">إلغاء</a>
            </div>
        </form>
    </main>
