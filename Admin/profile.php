...
// Fetch Admin user data from session
$fullName = htmlspecialchars($_SESSION['FullName'] ?? '');
$userRole = htmlspecialchars($_SESSION['UserRole'] ?? 'Admin');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'update_profile') {
    $_SESSION['AdminName'] = $_POST['Admin_name'] ?? $_SESSION['AdminName'];
    $_SESSION['FullName'] = $_POST['owner_name'] ?? $_SESSION['FullName'];
    ...
    $alert_message = 'تم تحديث ملف تعريف الشركة بنجاح!';
    $alert_type = 'success';
}
...
<main>
    <div class="container" style="max-width: 800px; margin-top: 40px; margin-bottom: 60px; text-align: right;">
        <h2 class="mb-4"><i class="fa-solid fa-user-gear text-primary" style="margin-left: 8px;"></i> ملف تعريف الشركة</h2>
        ...
        <div style="display: flex; flex-direction: column; gap: 30px;">
            ...
            <form method="POST" class="card" style="padding: 30px;">
                ...
                <h3 class="mb-4" style="font-size: 1.25rem; font-weight: 600; color: #1e293b; border-bottom: 1px solid #e2e8f0; padding-bottom: 10px;">
                    <i class="fa-solid fa-id-card text-accent" style="margin-left: 8px;"></i> معلومات الشركة
                </h3>
                
                <div class="form-group">
                    <label class="form-label" for="Admin_name">اسم الشركة</label>
                    <input type="text" id="Admin_name" name="Admin_name" class="form-control" value="<?php echo $AdminName ?: $fullName; ?>" required>
                </div>
                <div class="form-group">
                    <label class="form-label" for="owner_name">اسم المالك</label>
                    <input type="text" id="owner_name" name="owner_name" class="form-control" value="<?php echo $fullName; ?>" required>
                </div>
                <div class="form-group">
